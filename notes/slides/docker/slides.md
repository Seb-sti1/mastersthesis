...
RUN apt update && apt install git -y &&\
    git clone -b kerbourch-refactor-patch-6648 https://gitlab.ensta.fr/ssh/mirador /app &&\
    npm install
EXPOSE 3000
...
- Usually one docker of each software (e.g. Frontend NodeJS + Backend NodeJS + MySQL)
...
- `--device /dev/dri:/dev/dri --ipc=host -e DISPLAY=$DISPLAY
  -e XAUTHORITY=/root/.Xauthority -v /tmp/.X11-unix:/tmp/.X11-unix`: make X11 server (GUI) available (like ssh X11
  forwarding)
